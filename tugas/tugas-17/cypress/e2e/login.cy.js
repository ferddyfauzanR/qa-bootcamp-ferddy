...
describe('Modul Login - OrangeHRM', () => {
  ...
  context('Positive Case', () => {
    ...
    it('[TC-LOG-01] Login dengan kredensial valid', () => {
      ...
      LoginPage.dashboardBreadcrumb.should('contain', 'Dashboard');
    });
  ...
  context('Negative Case', () => {
    ...
    it('[TC-LOG-05] Login dengan kedua field kosong', () => {
      ...
      LoginPage.usernameRequiredMsg.should('be.visible').and('contain', 'Required');
      LoginPage.passwordRequiredMsg.should('be.visible').and('contain', 'Required');
    });
    ...
    it('[TC-LOG-08] Login dengan spasi di username (leading & trailing)', () => {
      ...
      LoginPage.login(data.usernameWithSpaces, data.validUser.password);

      // Expected Result: sistem menolak login, error "Invalid credentials"
      LoginPage.errorMessage.should('be.visible');
      LoginPage.errorMessage.should('contain', 'Invalid credentials');
    });
    ...
    it('[TC-LOG-09] Login dengan username huruf kecil semua (case-insensitive check)', () => {
      ...
      LoginPage.login(data.usernameLowercase, data.validUser.password);

      // Expected Result: user berhasil login (username tidak case-sensitive)
      cy.url().should('include', data.dashboardUrl);
      LoginPage.dashboardBreadcrumb.should('contain', 'Dashboard');
    });
  ...
  context('UI & Fitur Pendukung', () => {
    ...
    it('[TC-LOG-11] Klik link "Forgot your password?" diarahkan ke halaman reset', () => {
      ...
      cy.url().should('include', data.forgotPasswordUrl);
